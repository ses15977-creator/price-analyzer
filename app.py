...
# ---------------------------------------------------------
# 2. Supabase DB 및 회원가입/로그인 세션 관리
# ---------------------------------------------------------
...
try:
    ...
    st.error("Supabase 연동 설정(Secrets)을 확인해주세요.")
    st.stop()
...
if not st.session_state.user:
    st.markdown("<h2 style='text-align: center;'>🔐 SellMetrics Pro</h2>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #64748B;'>서비스 이용을 위해 로그인 또는 회원가입을 해주세요.</p>", unsafe_allow_html=True)
    ...
    with col_b:
        ...
        with tab_signup:
            ...
            new_email = st.text_input("이메일 주소", key="signup_email")
            new_password = st.text_input("비밀번호", type="password", key="signup_pw")
            user_name = st.text_input("성함 / 셀러명", key="signup_name")
            
            if st.button("회원가입 완료", use_container_width=True):
                if new_email and new_password and user_name:
                    try:
                        ...
                        st.success("회원가입이 완료되었습니다! [로그인] 탭으로 이동해 로그인해 주세요.")
                    except Exception as e:
    ...
    st.stop() # 로그인 전 메인 화면 접근 차단

# ---------------------------------------------------------
# 3. 메인 앱 시작 (로그인 성공 후)
# ---------------------------------------------------------
user_info = st.session_state.user.user_metadata
st.sidebar.write(f"👤 **{user_info.get('name', '셀러')}**님 환영합니다!")
...
if target_price <= lowest_price:
    ...
    summary_opinion = f"목표 판매가가 시장 최저가 대비 {price_diff:,}원 높습니다. 사은품 증정이나 세트 상품 구성을 통한 추가 가치 제고 전략이 필요합니다."

if cost_price >= lowest_price:
    summary_opinion += " 🚨 경고: 공급 원가가 시장 최저가 이상이므로 원가 인하 협상이 시급합니다."

# ---------------------------------------------------------
# 5. 리스크 평가 스코어카드 & 광고 역산 계산
# ---------------------------------------------------------
risk_score = 100
risk_reasons = []
...
_, avg_profit, _ = calculate_margin(target_price, cost_price, shipping_fee, fee_naver)
if avg_profit > 0 and target_price > 0:
    min_bep_roas = round((target_price / avg_profit) * 100, 1)
    max_cpc_100per = int(avg_profit)
    max_cpc_50per = int(avg_profit * 0.02)
else:
    min_bep_roas, max_cpc_100per, max_cpc_50per = 0, 0, 0
...
st.markdown("---")
tab_opt, tab_ad = st.tabs(["🎯 마켓별 역산 판매가 추천", "📢 스마트 광고 한도 & ROAS 역산기"])
...
with tab_opt:
    ...
    for p in platforms:
        ...
        rec_price = int(round(rec_price, -2))
...
with tab_ad:
    ...
    col_ad1.metric("최소 목표 ROAS (손익분기점)", f"{min_bep_roas}%")
    col_ad2.metric("최대 클릭당 단가 (전환율 1%)", f"{max_cpc_100per:,}원")
    col_ad3.metric("최대 클릭당 단가 (전환율 2%)", f"{max_cpc_50per:,}원")
...
<body>
    ...
    <table>
        <thead><tr><th>공급 원가</th><th>기본 배송비</th><th>목표 판매가</th><th>시장 최저가</th><th>최저가 대비 격차</th></tr></thead>
        <tbody><tr><td><b>{cost_price:,}원</b></td><td>{shipping_fee:,}원</td><td><b>{target_price:,}원</b></td><td>{lowest_price:,}원 ({lowest_platform})</td><td><b>{diff_text}</b></td></tr></tbody>
    </table>
    <div class="section-title">2. 채널별 마진 및 수익 분석</div>
    <table>
        <thead><tr><th>채널명</th><th>수수료율</th><th>공제 수수료</th><th>예상 순이익</th><th>마진율</th><th>진입 판정</th></tr></thead>
        <tbody>{margin_rows_html}</tbody>
    </table>
    <div class="section-title">3. 마케팅 집행 한도 (손익분기점)</div>
    <table>
        <thead><tr><th>최소 목표 ROAS</th><th>클릭당 최대 허용 단가 (전환율 1%)</th><th>클릭당 최대 허용 단가 (전환율 2%)</th></tr></thead>
        <tbody><tr><td><b>{min_bep_roas}%</b></td><td>{max_cpc_100per:,}원</td><td>{max_cpc_50per:,}원</td></tr></tbody>
    </table>
    <div class="section-title">4. 주요 플랫폼 경쟁 현황</div>
    <table>
        <thead><tr><th>플랫폼</th><th>상품명</th><th>판매가</th><th>리뷰 수</th></tr></thead>
        <tbody>{market_rows_html}</tbody>
    </table>
    <div class="section-title">5. 최종 시장 진입 소견</div>
    <div class="opinion-box"><b>[종합 의견]</b> {summary_opinion}</div>
</body>
</html>
"""
...
<div style="font-family: sans-serif; padding-top: 5px;">
    <button onclick="openPrintTab()" style="
        width: 100%; padding: 10px 12px; background-color: #0F172A; color: white;
...
with col_info:
    st.caption("💡 클릭 시 1페이지 규격의 요약 보고서 인쇄 창이 바로 나타납니다.")
